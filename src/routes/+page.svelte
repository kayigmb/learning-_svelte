...
<div class="flex items-center w-full justify-center gap-4 flex-wrap">
   <h1 class="text-center font-bold text-[25px] sm:text-[50px]">Welcome <span
      class="capitalize"> {data.info.name}</span> !</h1> 
   <button
      on:click={Logout} class=" w-[100px] h-[40px] text-white bg-black
      hover:bg-slate-700 
      ...
      <form method="POST" action="?/todo" class="w-full flex flex-col
         ...
         {#if form?.error && form?.error.todo._errors.length > 0}
            <small class="text-red-700
               text-[25px]">{form?.error?.todo?._errors[0]}</small> {/if}
         <button class="w-[90%] sm:w-[60%] text-[20px] border border-black
            bg-black text-white hover:bg-gray-900  sm:text-[30px] font-bold
...
<div class="w-[90%] sm:w-[60%] m-auto mt-8 min-h-[250px] shadow-lg border rounded-lg
   ...
   <div class=" m-auto p-4  flex flex-col items-center w-[100%] gap-5">
      ...
      {:else}
         {#each data.todos as data(data.id)} 
               <div  class="flex justify-between items-center  w-[90%] sm:w-[60%]">
                  <div class="flex items-center gap-8 h-[50px]"> 
                     ...
                     <p class:checked={data.completed}
                        class="font-bold">{data.todo}</p> </div>
                  <div class="flex flex-col sm:flex-row gap-4 sm:gap-4">   
                     <button class="w-[70px] sm:w-[100px] h-[40px] rounded-lg
                     bg-red-500 hover:bg-red-700 font-bold"
                        on:click={()=>deleteTodo({todoId:data.id})}>Delete</button>
               </div>
               </div>
